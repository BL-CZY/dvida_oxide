...
run: clean-img kernel $(IMAGE_NAME).iso
	qemu-system-x86_64 -m 4G -boot d -cdrom $(IMAGE_NAME).iso -drive file=storage.img,format=raw,media=disk -s -serial stdio -machine q35 -smp 4 -bios ovmf/OVMF.fd -d int
